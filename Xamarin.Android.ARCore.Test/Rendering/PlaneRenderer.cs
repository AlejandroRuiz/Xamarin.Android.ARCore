...
namespace Xamarin.Android.ARCore.Test.Rendering
{
    ...
    {
        private const string TAG = nameof(PlaneRenderer);

        private const int BYTES_PER_FLOAT = Float.Size / 8;
        private const int BYTES_PER_SHORT = Short.Size / 8;
        private const int COORDS_PER_VERTEX = 3; // x, z, alpha

        private const int VERTS_PER_BOUNDARY_VERT = 2;
        private const int INDICES_PER_BOUNDARY_VERT = 3;
        private const int INITIAL_BUFFER_BOUNDARY_VERTS = 64;

        private const int INITIAL_VERTEX_BUFFER_SIZE_BYTES =
            BYTES_PER_FLOAT * COORDS_PER_VERTEX * VERTS_PER_BOUNDARY_VERT *
                INITIAL_BUFFER_BOUNDARY_VERTS;

        private const int INITIAL_INDEX_BUFFER_SIZE_BYTES =
            BYTES_PER_SHORT * INDICES_PER_BOUNDARY_VERT * INDICES_PER_BOUNDARY_VERT *
                INITIAL_BUFFER_BOUNDARY_VERTS;

        private const float FADE_RADIUS_M = 0.25f;
        private const float DOTS_PER_METER = 10.0f;
        private float EQUILATERAL_TRIANGLE_SCALE = (float)(1 / System.Math.Sqrt(3));
        ...
        private float[] GRID_CONTROL = { 0.2f, 0.4f, 2.0f, 1.5f };
        ...
        private Dictionary<Plane, Integer> mPlaneIndexMap = new Dictionary<Plane, Integer>();
            ...
            FloatBuffer boundary)
        {
            ...
            // out ok.
            float xScale = System.Math.Max((extentX - 2 * FADE_RADIUS_M) / extentX, 0.0f);
            float zScale = System.Math.Max((extentZ - 2 * FADE_RADIUS_M) / extentZ, 0.0f);
        ...
        private void Draw(float[] cameraView, float[] cameraPerspective)
        {
            ...
            global::Android.Opengl.Matrix.MultiplyMM(mModelViewMatrix, 0, cameraView, 0, mModelMatrix, 0);
            global::Android.Opengl.Matrix.MultiplyMM(mModelViewProjectionMatrix, 0, cameraPerspective, 0, mModelViewMatrix, 0);
         ...
         */
        public void DrawPlanes(ICollection<Plane> allPlanes, Pose cameraPose,
                float[] cameraPerspective)
        {
            ...
            sortedPlanes.Sort((SortablePlane a, SortablePlane b) =>
            {
            ...
            foreach (SortablePlane sortedPlane in sortedPlanes)
            {
                ...
                Integer planeIndex = mPlaneIndexMap[plane];
                if (planeIndex == null)
                {
                    planeIndex = Integer.ValueOf(mPlaneIndexMap.Count);
                    mPlaneIndexMap[plane] = planeIndex;
                }

                // Set plane color. Computed deterministically from the Plane index.
                var colorIndex = (int)planeIndex % PLANE_COLORS_RGBA.Length;
                ...
                float angleRadians = (int)planeIndex * 0.144f;
                float uScale = DOTS_PER_METER;
                float vScale = DOTS_PER_METER * EQUILATERAL_TRIANGLE_SCALE;
                mPlaneAngleUvMatrix[0] = +(float)System.Math.Cos(angleRadians) * uScale;
                mPlaneAngleUvMatrix[1] = -(float)System.Math.Sin(angleRadians) * uScale;
                mPlaneAngleUvMatrix[2] = +(float)System.Math.Sin(angleRadians) * vScale;
                mPlaneAngleUvMatrix[3] = +(float)System.Math.Cos(angleRadians) * vScale;
        ...
        private uint[] PLANE_COLORS_RGBA = new uint[]{
        0xFFFFFFFF,
        0xF44336FF,
        0xE91E63FF,
        0x9C27B0FF,
